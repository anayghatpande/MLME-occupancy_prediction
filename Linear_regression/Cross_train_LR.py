...
dataset1 = dataset[0:127]

Floor_heads2 = list(df_building_clean.get(val2).columns)

df_building_clean[val2] = df_building_clean.get(val2).drop(columns = ['Time'])
df_building_clean.get(val2).insert(1, 'PIR_ALL',df_building_clean.get(val2)[Floor_heads2[1:27]].sum(axis=1), True)
df_building_clean.get(val2).insert(1, 'CO2_ALL',df_building_clean.get(val2).reindex(columns=Floor_heads2[28:53]).sum(axis=1), True)
min_max_scaler = preprocessing.MinMaxScaler()
x_scaled1 = min_max_scaler.fit_transform(df_building_clean.get(val2)[['CO2_ALL']])
df_building_clean.get(val2).insert(1, 'CO2_ALL_scaled',x_scaled1, True)

dataset2 = df_building_clean[val2]


#AP_Total
x1 = dataset1["AP_Total"].values.reshape(-1, 1)
y1 = dataset2["AP_Total"].values.reshape(-1, 1)
...
plt.title('AP_Total '+' R2 value '+str(r2_score(y1, y1_pred)))
plt.xlabel("AP_Total Floor1S")
plt.ylabel("AP_Total Floor1W")
plt.show()

#"Inst_kW_Load_Plug"
x1_1 = dataset1["Inst_kW_Load_Plug"].values.reshape(-1, 1)
y1_1 = dataset2["Inst_kW_Load_Plug"].values.reshape(-1, 1)
...
plt.title('Inst_kW_Load_Plug '+' R2 value '+str(r2_score(y1_1, y1_1_pred)))
plt.xlabel("Inst_kW_Load_Plug 1S")
plt.ylabel("Inst_kW_Load_Plug 1W")
plt.show()


#PIR_ALL
x1_2 = dataset1["PIR_ALL"].values.reshape(-1, 1)
y1_2 = dataset2["PIR_ALL"].values.reshape(-1, 1)
...
plt.xlabel("PIR_ALL 1S")
plt.ylabel("PIR_ALL 1W")
plt.show()

#CO2_ALL
x2 = dataset1["CO2_ALL"].values.reshape(-1, 1)
y2 = dataset2["CO2_ALL"].values.reshape(-1, 1)
...
plt.title("CO2_ALL " +' R2 value '+str(r2_score(y2, y2_pred)))
plt.xlabel("CO2_ALL 1S")
plt.ylabel("CO2_ALL 1W")
plt.show()

#"AP_Total","Inst_kW_Load_Plug"
x3 = dataset1[["AP_Total","Inst_kW_Load_Plug"]].values
y3 = dataset2[["AP_Total","Inst_kW_Load_Plug"]].values
...
x4 = dataset1[["AP_Total","Inst_kW_Load_Plug","PIR_ALL"]].values
y4 = dataset2[["AP_Total","Inst_kW_Load_Plug","PIR_ALL"]].values
...
x5 = dataset1[["AP_Total","Inst_kW_Load_Plug","PIR_ALL","CO2_ALL"]].values
y5 = dataset2[["AP_Total","Inst_kW_Load_Plug","PIR_ALL","CO2_ALL"]].values
