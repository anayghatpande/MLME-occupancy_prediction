...
X = df_building_clean.get('Floor1S')[['AP_Total','Inst_kW_Load_Plug','PIR_ALL','CO2_ALL_scaled']].values
X = (np.atleast_2d(X))
...
X2 = df_building_clean.get('Floor1W')[['AP_Total','Inst_kW_Load_Plug','PIR_ALL','CO2_ALL_scaled']].values
X2 = (np.atleast_2d(X2))
...
gp = GaussianProcessRegressor(kernel=kernel,optimizer='fmin_l_bfgs_b',alpha=0.1, n_restarts_optimizer=20,normalize_y=False)
...
plt.title('Model_gauss 1st-FloorS vs. Data 1st-FloorS (AP+kW+PIR+C_scaled), [R_2:%s] ' %w_2, fontsize=15, color= 'black', y= 1.1)
...
plt.figure(figsize=(12,5))
plt.title('Model_gauss 1st-FloorS vs. Data 1st-FloorW (AP+kW+PIR+C_scaled), [R_2:%s] ' %w_2, fontsize=15, color= 'black', y= 1.1)
