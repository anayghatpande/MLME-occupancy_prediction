...
df_building_clean['Floor1S'] = df_building_clean.get('Floor3').drop(columns = ['AP 1','AP 2'])
df_building_clean['Floor1S'] = df_building_clean.get('Floor3').drop(columns = ['Inst_kW_Load_Light','Inst_kW_Load_Elec'])
...
df_building_clean.get('Floor3').insert(1, 'CO2_ALL',df_building_clean.get('Floor3').loc[:,Floor3_heads[23:42]].sum(axis=1), True)
...
X = df_building_clean.get('Floor3')[['AP_Total','Inst_kW_Load_Plug']].values
X = (np.atleast_2d(X))
...
X2 = df_building_clean.get('Floor1W')[['AP_Total','Inst_kW_Load_Plug']].values
X2 = (np.atleast_2d(X2))
...
plt.title('Model_gauss 3rd-FloorS vs. Data 3rd-FloorS (AP+kW), [R_2:%s] ' %w_2, fontsize=15, color= 'black', y= 1.1)
...
plt.figure(figsize=(12,5))
plt.title('Model_gauss 3rd-FloorS vs. Data 1st-FloorW (AP+kW), [R_2:%s] ' %w_2, fontsize=15, color= 'black', y= 1.1)
