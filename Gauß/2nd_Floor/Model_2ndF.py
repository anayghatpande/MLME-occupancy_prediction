# -*- coding: utf-8 -*-
"""
Created on Sat Jul 10 16:28:08 2021
...
Time_2W = df_building_clean.get('Floor2S')[['Time']]
...
df_building_clean.get('Floor2W').insert(0, 'PIR_ALL',df_building_clean.get('Floor2W').loc[:,Floor2W_heads[1:11]].sum(axis=1), True)
df_building_clean.get('Floor2W').insert(1, 'CO2_ALL',df_building_clean.get('Floor2W').loc[:,Floor2W_heads[12:24]].sum(axis=1), True)


X11 = df_building_clean.get('Floor2S')[['Inst_kW_Load_Plug','AP_Total','PIR_ALL','CO2_ALL']].values
X12 = df_building_clean.get('Floor2S')[['Inst_kW_Load_Plug','AP_Total','PIR_ALL']].values
X13 = df_building_clean.get('Floor2S')[['Inst_kW_Load_Plug','AP_Total']].values
X = df_building_clean.get('Floor2S')[['AP_Total']].values
X = (np.atleast_2d(X))


y = df_building_clean.get('Floor2S')[['Groundtruth']].iloc[:,0].values
y = y.reshape(-1,1)
x = (np.atleast_2d(np.linspace(1,100,len(Time_2W)))).T

X2 = df_building_clean.get('Floor2W')[['Inst_kW_Load_Plug','AP_Total','PIR_ALL','CO2_ALL']].values
X22 = df_building_clean.get('Floor2W')[['Inst_kW_Load_Plug','AP_Total','PIR_ALL']].values
X23 = df_building_clean.get('Floor2W')[['Inst_kW_Load_Plug','AP_Total']].values
X2 = df_building_clean.get('Floor2W')[['AP_Total']].values
X2 = (np.atleast_2d(X2))
y2 = df_building_clean.get('Floor2W')[['Groundtruth']].iloc[:,0].values
y2 = y2.reshape(-1,1)
x2 = (np.atleast_2d(np.linspace(1,100,155))).T
...
k1 =  RBF(length_scale=1)  # long term smooth rising trend
...
plt.title('Model_gauss 2nd-FloorS vs. Data 2nd-FloorS (AP+kW+PIR+C), [R_2:%s] ' %w_2, fontsize=15, color= 'black', y= 1.1)
...
plt.figure(figsize=(12,5))
plt.title('Model_gauss 2nd-FloorS vs. Data 2nd-FloorW (AP+kW+PIR+C), [R_2:%s] ' %w_2, fontsize=15, color= 'black', y= 1.1)
